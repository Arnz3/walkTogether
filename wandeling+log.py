...
csv_file = csv.reader(open('test 00.csv')) #read the csv file
csv_list = list(csv_file)   #convert file to list

f = open('/home/pi/Downloads/gps/punten_wandeling', 'w')
writer =  csv.writer(f)
...
def getCurrentLocation():
    ...
    writer.writerow([time,lat_gps,lon_gps])
...
try:
    ...
    while True:
        ...
        if abs(lon_gps - lon_csv) < max_difference and abs(lat_gps - lat_csv) < max_difference:
            time = datetime.now().strftime("%H:%M:%S")
            writer.writerow([time,lat_gps,lon_gps,lat_csv,lon_csv])
            print("point")
            turn(direction)
